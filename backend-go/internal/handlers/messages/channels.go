	...
	return func(c *gin.Context) {
		...
		for i, up := range cfg.Upstream {
			...
			upstreams[i] = gin.H{
				...
				"routeMessagesToResponsesPool":     up.IsRouteMessagesToResponsesPoolEnabled(),
				"bridgedToResponsesPool":           strings.EqualFold(up.ServiceType, "responses") && up.IsRouteMessagesToResponsesPoolEnabled(),
...
func GetChannelModels(cfgManager *config.ConfigManager) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if useManualModels {
			...
			log.Printf("[Messages-Models] 返回手工模型列表: channel=%s, count=%d", channelName, len(manualModels))
