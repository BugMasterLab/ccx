...
- **修复能力测试无 API Key 时的伪协议问题** - 后端移除无 API Key 时返回的 `all` 伪协议，改为直接返回 `failed` 状态和 `no_api_key` 错误；前端增加已知协议白名单过滤，避免非法协议被渲染
- **优化能力测试错误提示** - 能力测试 job 失败时弹窗直接进入错误态，特殊处理 `no_api_key` 错误显示明确提示，其他错误显示通用失败信息

### Changed

- **精简前端对话框样式** - 将重复的 `.dialog-title` 样式提取到全局 CSS (`style.css`)，删除 `ChannelLogsDialog` 和 `CapabilityTestDialog` 组件内的重复定义
- **清理能力测试残留流程** - 删除前端 API 层残留的”可选模型/可选目标协议”参数 (`models`, `targetProtocols`)，移除 `CapabilityTestJob` 类型中的 `targetProtocols` 字段，恢复”进入后直接列出所有协议和模型”的原始流程

- **微调前端字号层级与弹窗标题权重** - 下调”编辑渠道”弹窗头部标题与副标题层级，并统一等宽字体下 `text-caption` / `text-body-*` 的小字行高，避免头重脚轻与说明文字发虚
- **提升前端辅助信息可读性** - 调整渠道编排页序号角标、服务类型标签、RPM/TPM 标签、状态徽标、日志/能力测试文本与图表 tooltip 字号，在保持紧凑布局的前提下改善可读性
